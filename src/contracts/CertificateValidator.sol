...
contract CertificateValidator {
    ...
    function getCertificateDetails(string memory documentId) public view returns (
        string memory,
        address,
        uint256,
        bool
    ) {
        Certificate memory cert = certificates[documentId];
        return (cert.documentId, cert.issuer, cert.timestamp, cert.isValid);
    }
} 